...
The cool thing about having implemented this cache using Git is that we get a well-tested replication protocol for free. This allows having a decentralized setup where multiple caches share packages with each other. To fetch a package from another replica, we can simply run `git fetch remote_peer /refs/<nix-hash>/pkg:/refs/<nix-hash>/pkg`. This command copies the reference `/refs/<nix-hash>/pkg` and fetches all objects from the remote peer which are reachable from it. (There are some quirks to it, which is explained in my [bachelor thesis](https://cn.dmi.unibas.ch/fileadmin/user_upload/redesign-cn-dmi/pubs/theses/bachelor/Siegfried-Gachix-Nix-Bin-Cache-Over-Git.pdf)).

But how are packages added to this network of Gachix peers? Gachix peers can also communicate with remote or local Nix daemons. The Nix daemon is a service which runs Nix specific operations on behalf of non-root users. It allows Gachix to use the Nix API. We can fetch Nix archives using this API. We convert these NARs to Git objects, construct the commit objects and place the necessary references as specified above. Given the dependency graph of a package, we can construct the Git objects by exploring the graph in a depth-first-search manner.

As of now, Gachix doesn't have a clear policy on when packages are added to the cache. The cache admin needs run a command to add a package. There is a hardcoded order on when which peer or daemon is contacted. This can definitely be enhanced by designing a peer-selection heuristic which minimizes the latency of package addition.
